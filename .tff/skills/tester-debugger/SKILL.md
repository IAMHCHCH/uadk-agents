...
- **记忆**：你记住：
  - hisi_zip SQE 结构体（`drv/hisi_comp.c:170-232`）：32 双字，关键字段包括 consumed（dw0）、produced（dw1）、comp_data_length（dw2）、status（dw3:0-7）、input_data_length（dw4）、dest_avail_out（dw14）
  ...
  - 远程服务器：`ssh root@192.168.90.141`，开发路径：`/root/hch/lz4_uadk/`
...
```bash
# 基本往返测试
cd /root/hch/lz4_uadk && LD_LIBRARY_PATH=/usr/local/lib ./test_lz4_uadk

# 带 BD dump 的调试运行
WD_BD_DUMP=1 LD_LIBRARY_PATH=/usr/local/lib ./test_lz4_uadk 2>&1 | tee bd_dump.log

# 性能剖析
perf record -g LD_LIBRARY_PATH=/usr/local/lib ./bench_lz4_uadk
perf report

# NUMA 绑定单设备测试
numactl --cpunodebind=0 --membind=0 LD_LIBRARY_PATH=/usr/local/lib ./bench_lz4_uadk

# 双设备测试（无 NUMA 绑定）
numactl -n -1 LD_LIBRARY_PATH=/usr/local/lib ./bench_lz4_uadk
...
```bash
# 基本正确性测试
ssh root@192.168.90.141 "cd /root/hch/lz4_uadk && \
...
### 第三步：失败调试
如果测试失败：
1. 启用 BD dump 重现：`WD_BD_DUMP=2 ./test_lz4_uadk 2>&1 | tee bd_dump.log`
2. 分析 SQE dump：
   - 检查 status 字段的错误码
...
ssh root@192.168.90.141 "cd /root/hch/lz4_uadk && \
  ...
  LD_LIBRARY_PATH=/usr/local/lib ./bench_lz4_uadk"

# 调用图剖析
perf record -g -F 99 LD_LIBRARY_PATH=/usr/local/lib ./bench_lz4_uadk
